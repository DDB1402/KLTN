  ...
  public searchUserByEmailOrPhone(keyword: Maybe<string>, id_user: string) {
    ...
    >((resolve, reject) => {
      const query = this.db.query(
        ...
        FROM (SELECT ${queryInfoString} FROM user WHERE (name LIKE CONCAT('%', ?,  '%') OR email LIKE CONCAT('%', ?,  '%') OR phone LIKE CONCAT('%', ?,  '%')) AND delFlag=${DEL_FLAG.VALID} AND id_user!=?)
        ...
        `,
        [id_user,keyword, keyword, keyword, id_user, id_user, id_user, id_user,id_user],
        (err, result) => {
          if (err) reject(err);
          else
            resolve(
...
public insertNewStatusBetween(id_user:string,id_friend:string,status:FRIEND_STATUS){
  ...
  public lockUser(id_user: string){
    return new Promise<any>((resolve, reject) => {
      this.db.query(
        `
        UPDATE user 
  ...
  public unLockUser(id_user: string){
    return new Promise<any>((resolve, reject) => {
      this.db.query(
        `
        UPDATE user 
